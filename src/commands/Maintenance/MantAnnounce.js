	...
	async run(message, args) {
		if (message.member.hasPermission('MANAGE_MESSAGES')) {
			if (config.maintenanceStatus === 'none') {
				...
				config.maintenanceStatus = 'announced';
			} else { return message.channel.send(`No se ha podido anunciar el mantenimiento. El estado del mantenimiento es ${config.maintenanceStatus.toUpperCase()}. Requiere: **NONE**`); }
		}
	}
